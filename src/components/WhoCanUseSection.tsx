    ...
    <section id="who-can-use" className="section-padding relative">
      ...
      <div ref={ref} className="container mx-auto relative z-10">
        <div className={`text-center mb-8 transition-all duration-700 ${isVisible ? "animate-fade-up" : "opacity-0"}`}>
          ...
          <p className="max-w-2xl mx-auto mt-6 text-muted-foreground text-lg">{users.description}</p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {users.cards.map((user, i) => (
            <div
              key={user.id}
              className={`rounded-2xl overflow-hidden card-hover group transition-all duration-700 relative ${isVisible ? "animate-fade-up" : "opacity-0"}`}
              style={{ animationDelay: `${i * 80}ms` }}
            >
              <div className="relative h-56 overflow-hidden rounded-2xl">
                <img src={user.image} alt={user.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy" />
